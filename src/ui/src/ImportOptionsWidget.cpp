...
ImportOptionsWidget::ImportOptionsWidget(QWidget* parent) : QWidget(parent) {
  ...
  mode_->addItem(tr("Visual-relative"),
                 static_cast<int>(cad::TessellationMode::VisualRelative));
  mode_->addItem(tr("Absolute"),
                 static_cast<int>(cad::TessellationMode::Absolute));
  ...
  connect(mode_, &QComboBox::currentIndexChanged, this, [this, edited](int) {
    update_enablement();
    edited();
  });
  for (auto* spin : {linear_, angular_, target_px_, reference_px_,
                     min_deflection_, max_relative_}) {
...
void ImportOptionsWidget::update_enablement() {
...
cad::ImportOptions ImportOptionsWidget::options() const {
  cad::ImportOptions o;
...
void ImportOptionsWidget::set_options(const cad::ImportOptions& o) {
  applying_ = true;
